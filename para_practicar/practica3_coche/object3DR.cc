...
void _object3DR::revolucion(unsigned int num_rotaciones) {

    if(num_rotaciones < 3) num_rotaciones = 3;  //Se necesitan por lo menos 3 perfiles para generar un objeto 3D

    // Guardar los puntos de los extremos, ya que los vamos a almacenar al final del vector (en caso de doble corte)
    ...
    // El numero de trinagulos se puede calcular como puntos_medios*num_rotaciones*2
    ...
    // Aniadimos los triangulos del bloque central
    for (unsigned int i = 0; i < (puntos_medios - 1); i++){
        for (unsigned int j = 0; j < num_rotaciones; j++){
            ...
            //cout << "(bloque central 0) Triangulo " << h << ": " << A << ", " << B << ", " << C << endl;
            //cout << "(bloque central 1) Triangulo " << h+1 << ": " << B << ", " << C << ", " << D << endl;
    ...
    if(tapa_abajo) {
        for (unsigned int i = 0; i < num_rotaciones; i++){
            ...
            //cout << "(parte inferior) Triangulo " << h << ": " << A << ", " << B << ", " << C << endl;
    ...
    if (tapa_arriba) {
        ...
        for (unsigned int i = 0; i < num_rotaciones; i++){
            ...
            //cout << "(parte superior) Triangulo " << h << ": " << A << ", " << B << ", " << C << endl;
            if(arriba_abajo) {
                Triangles[h] = _vertex3ui(C, B, A);
            }
            else {
